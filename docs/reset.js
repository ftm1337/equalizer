	...
	if (bal == 0) {
		...
		for(i=0;i<bal;i++) {
			$("nft-sel").innerHTML += `
				<div class="mechanism">
					<div class="twocol">
						<div>Token ID</div> <div>#${nids[i]}</div>
...
async function reset(_id) {
	...
	if(alvo[0]!=0) {
		notice(`
			<h3>Attached veNFT!</h3>
			Your veNFT is currently attached to ${alvo[0]} gauge deposits.
			<br>
			<h4><u><i>Please unstake your LPs first and try again.</i></u></h4>
		`);
	}
	if(alvo[1]==true) {
		notice(`
			<h3>Votes Detected - Resetting your Votes!</h3>
			We detected that veNFT #${_id} has voted previously at Equalizer.
			<br>Resets are necessary to be able to move the veNFT. Each reset will count as a Vote. To be able to revote with this veNFT, you will need to merge it into newer veNFT.<br>Note: VeNFTs that have already voted in the ongoing epoch cannot be reset in this epoch.
			<br><br>
		...
		notice(`
			...
			<h4><a target="_blank" href="${EXPLORE}/tx/${_tr.hash}">View on Explorer</a></h4>
			<br><br>
			Your veNFT #${_id} can now be moved around freely!
		`);
	}
	gubs()
}
